...
-- PRIMEIRO FAZEMOS A CHECAGEM PARA VER QUANTAS INSTANCIAS TEM NO SEXO FEMININO
select count(*), sexo
from funcionarios
group by sexo;
...
-- DEPOIS, FAZEMOS A CONTAGEM/AGRUPAMENTO DOS DEPARTAMENTOS
-- PARA SABER QUAL TEM + E ASSIM GARANTIR A PERFORMANCE DA CONSULTA
...
-- DEPARTAMENTO TEM MENOS CHANCE DE ENCONTRAR DO QUE O SEXO
-- E COMO ESTAMOS USANDO A CONDICIONAL AND, DEVE-SE COLOCAR 1º O QUE TEM MENOS CHANCE
-- NO CASO O DEPARTAMENTO TEM MENOS CHANCE QUE O SEXO FEMININO QUE É 491
-- JÁ AO USAR O OR DEVE-SE USAR 1º O QUE TEM MAIS CHANCE DE SER ENCONTRADO
-- E O LAR TEM + CHANCES DE SER ENCONTRADO DO QUE FILMES

--Traga os funcionarios do sexo masculino
--ou os funcionarios que trabalhem no setor Jardim

-- PARA PROJETAR A QUANTIDADE DOS SEXOS
SELECT SEXO, COUNT(*) FROM FUNCIONARIOS
GROUP BY SEXO;

-- MASCULINO = 484

-- PARA PROJETAR A QTDADE DE FUNCIONARIOS QUE TRABALHAM NO SETOR JARDIM
SELECT DEPARTAMENTO, COUNT(*)
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO;

-- DEPARTAMENTO JARDIM = 47

-- ENTÃO O EXERCÍCIO FICARIA ASSIM:
SELECT NOME, EMAIL
FROM FUNCIONARIOS
WHERE SEXO = 'MASCULINO'
OR
DEPARTAMENTO = 'JARDIM';
-- AO USAR O OR COLOCAMOS NA PRIMEIRA CONDICIONAL A VARIAVEL QUE TEM MAIS CHANCE DE SER TRUE
-- MASCULINO = 484 E DEPARTAMENTO = 'JARDIM' = 47
...
-- FORMA ERRADA, IRÁ RETORNAR QUERY VAZIA POIS NÃO É POSSÍVEL COMPARAR ALGO 'VAZIO'
SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
WHERE EMAIL = NULL;

-- FORMA CORRETA, IRÁ RETORNAR OS E-MAILS QUE SÃO NULL
SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
WHERE EMAIL IS NULL;

-- FORMA QUE IRÁ RETORNAR OS EMAILS QUE NÃO SÃO NULLs
SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
...
DELETE FROM CLIENTE; 
/* SE USAR O DELETE SEM WHERE, IRÁ FAZER = O UPDATE
SÓ QUE DESSA VEZ, IRÁ DELETAR TUDO DO BANCO */

SELECT * FROM CLIENTE WHERE NOME = 'ANA';
-- POR ISSO É 'OBRIGATÓRIO' USAR O WHERE JUNTO COM O DELETE
-- PARA GARANTIR QUE IRÁ DELETAR SOMENTE O REGISTRO DESEJADO

SELECT COUNT(*) FROM CLIENTE; -- TOTAL 6
-- TAMBÉM É ACONSELHADO FAZER UMA CONTAGEM ANTES E DEPOIS DA DELEÇÃO
-- PARA GARANTIR QUE O NUMERO DELETADO FOI O NUMERO DESEJADO
